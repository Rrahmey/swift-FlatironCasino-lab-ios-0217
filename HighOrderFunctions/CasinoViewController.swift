...
class CasinoViewController: UIViewController {
    ...
    func createDeck() {
        ...
        undealtCards = Rank.all.flatMap({ (rank: Rank) in
            Suit.all.map({ (suit: Suit) -> Card in
    ...
    func playCard(_ sender: UITapGestureRecognizer) {
        guard let lastCard = undealtCards.popLast() else {return}
    ...
    override func prepare(for segue: UIStoryboardSegue, sender: Any?) {
        if let destVC = segue.destination as? CardDetailViewController {
